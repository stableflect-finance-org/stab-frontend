...
import { Button, useModal, IconButton, AddIcon, MinusIcon } from '@stableflect/uikit'
import UnlockButton from 'components/UnlockButton'
import { useWeb3React } from '@web3-react/core'
import { useStabUser } from 'state/hooks'
...
const Staked: React.FunctionComponent<StabWithStakedValue> = ({ pid, lpSymbol, lpAddresses, quoteToken, token }) => {
  const TranslateString = useI18n()
  const { account } = useWeb3React()
  const [requestedApproval, setRequestedApproval] = useState(false)
  const { allowance, tokenBalance, stakedBalance } = useStabUser(pid)
